function pushToGitHub
    ...
    % Load information ragarding which probes were run by the validateAll.m script.
    % The following variables are loaded: 'sectionNames', 'validationScriptFileNames'
    load('ValidationProbesRun.mat');
    
    % We must read info for each section
    % sprintf('%s', fileread('info.txt'))
    
    % Empty all entries
    allEntries = [];
    for probeIndex = 1:numel(validationScriptFileNames)
        validationScriptFileName = validationScriptFileNames{probeIndex};
        sectionName  = sectionNames{probeIndex};
        [validationScriptDirectory, ~, ~] = fileparts(which(sprintf('%s.m',validationScriptFileName)));
        fprintf('\nAdding result from %s/%s.m', validationScriptDirectory, validationScriptFileName);
        
        entry.sectionLabelAndSubDirName = {sectionName, validationScriptDirectory};
        entry.validationScriptFileName  = validationScriptFileName;
